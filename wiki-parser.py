...
class WikiParser(xml.sax.ContentHandler):
    ...
    def endElement(self,tag):
        ...
        if tag == "page":
            ...
            iD = self.ide[0]
            curr_path = os.getcwd()
            files_dir = curr_path+'/intermediate_files/'
            ...
            #if self.count!=0:
            #    MegaDocStringkeys = sorted(self.MegaDocString.keys())
...
def main():
    ...
    #remaining files
    merge1000Files(intermediate_files_path,files_dir_path,str(Handler.batch +1),1000)
    files = glob.glob('intermediate_files/*')
    for f in files:
        os.remove(f)
    ...
    #############################################
    offset_title_path = './Titleoffset.txt'
    TitleList = Handler.titleList
    offset_title_file = open(offset_title_path,"w")
    filename = open(title_path,"w")
    for val in TitleList:
        #count = count + 1
        offset_title_file.write(str(filename.tell())+"\n")
        filename.write(val+"\n")
    filename.close()
    offset_title_file.close()
    #############################################

    
    #print(stop - start)
    start = time.time()
    indexName = open(path_to_index+'index1.txt','w')

    mergeFiles(files_dir_path,merged_files_path,"_",1000)
    mergeSimilarWords(indexName, merged_files_path)
